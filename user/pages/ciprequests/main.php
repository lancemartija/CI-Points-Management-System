    ...
    <main>
      ...
      <div class="items-center justify-between block p-4 bg-white border-b border-gray-200 sm:flex">
        <div class="w-full mb-1">
          <div class="mb-4">
              ...
              <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                  <div class="flex items-center">
                    ...
                    <a href="#" class="ml-1 text-sm font-medium text-gray-700 hover:text-gray-900 md:ml-2">CIP Requests</a>
                  </div>
                </li>
                <li>
                  <div class="flex items-center">
            ...
            <h1 class="mr-2 text-xl font-semibold text-gray-900 sm:text-2xl">All CIP Requests</h1>
          </div>
          <div class="sm:flex">
            <div class="items-center hidden mb-3 sm:flex sm:divide-x sm:divide-gray-100 sm:mb-0">
              <form class="flex lg:pr-2" action="#" method="get">
                <input type="hidden" name="id" value="<?= $_SESSION['userid']; ?>" readonly>
              ...
              <div class="flex pl-0 mt-3 space-x-1 sm:pl-2 sm:mt-0">
                <a href="ciprequests.php?id=<?= $_SESSION['userid']; ?><?= (isset($_GET['filter'])) ? '&filter=' . $_GET['filter'] : '' ?>" class="text-gray-900 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 border border-gray-200 font-medium inline-flex items-center rounded-lg text-base px-3 py-2 text-center;">
            ...
            <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
              <form action="../includes/export.inc.php?id=<?= $_SESSION['userid'] ?>" method="POST" enctype="multipart/form-data">
                <button type="submit" name="ci_export" class="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 sm:w-auto">
      ...
      <form method=" get">
        <ul class="flex flex-wrap m-3 text-sm font-medium text-center text-gray-500">
          <li class="mr-2">
            <a href="ciprequests.php?id=<?= $_SESSION['userid']; ?>" class="inline-flex px-4 py-3 rounded-lg <?= (isset($_GET['filter'])) ? 'hover:text-gray-900 hover:bg-gray-200' : 'text-gray-900 bg-gray-200'; ?>">
          ...
          <li class="mr-2">
            <a href="ciprequests.php?id=<?= $_SESSION['userid']; ?>&filter=pending" class="inline-flex px-4 py-3 rounded-lg <?= (isset($_GET['filter']) && $_GET['filter'] == 'pending') ? 'text-gray-900 bg-gray-200' : 'hover:text-gray-900 hover:bg-gray-200'; ?>">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
          ...
          <li class="mr-2">
            <a href="ciprequests.php?id=<?= $_SESSION['userid']; ?>&filter=approved" class="inline-flex px-4 py-3 rounded-lg <?= (isset($_GET['filter']) && $_GET['filter'] == 'approved') ? 'text-gray-900 bg-gray-200' : 'hover:text-gray-900 hover:bg-gray-200'; ?>">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
          ...
          <li class="mr-2">
            <a href="ciprequests.php?id=<?= $_SESSION['userid']; ?>&filter=rejected" class="inline-flex px-4 py-3 rounded-lg <?= (isset($_GET['filter']) && $_GET['filter'] == 'rejected') ? 'text-gray-900 bg-gray-200' : 'hover:text-gray-900 hover:bg-gray-200'; ?>">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
      ...
      if (!empty($records)) {
        if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
          ...
          if (isset($_GET['filter'])) {
            $records = $ciprequests->getCIPRequestFilteredSearchData($_SESSION['userid'], $_GET['filter'], $query, $start_from, $results_per_page);
          } else {
            $records = $ciprequests->getCIPRequestSearchData($_SESSION['userid'], $query, $start_from, $results_per_page);
          }
          ...
          $query = $_GET['filter'];
          $records = $ciprequests->getFilteredCIPRequestData($_SESSION['userid'], $query, $start_from, $results_per_page);
        ...
        include_once '../pages/ciprequests/tabledata.php';
      }
      ?>
    </main>
  </div>
