...
class DisplayData extends Dbh
{
  ...
  {
    $sql = 'SELECT u.user_id, u.first_name, u.middle_name, u.last_name, ur.request_status, COUNT(ur.activity_id) AS total FROM user_request ur LEFT JOIN user u ON ur.user_id = u.user_id GROUP BY ur.user_id ORDER BY u.last_name DESC;';
  ...
  {
    $stmt = $this->connect()->prepare('SELECT u.user_id, u.first_name, u.middle_name, u.last_name, ur.request_status, COUNT(ur.activity_id) AS total FROM user_request ur LEFT JOIN user u ON ur.user_id = u.user_id WHERE ur.request_status = ? GROUP BY ur.user_id ORDER BY u.last_name DESC;');
    $result = 0;
